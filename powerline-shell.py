...
class Powerline:
    ...
    def draw(self):
        ...
        # sys.stdout.write('length' + str(len(self.segments)))
        if len(self.segments) > 5:
            #sys.stdout.write(str(len(self.segments)))
            limit = range(len(self.segments)-1, len(self.segments)-5, -1)
            limit.reverse()
            #limit = range(3, len(self.segments))
            limit.insert(0, 1)
            # reversed((range(len(self.segments)-1, 4, -1)).append(0)))
            return (''.join(self.draw_segment(i) for i in limit)
                    + self.reset).encode('utf-8') + ' '
        else:
            return (''.join(self.draw_segment(i) for i in range(1, len(self.segments)))
                    + self.reset).encode('utf-8') + ' '
